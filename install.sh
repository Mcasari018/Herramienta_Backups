#!/usr/bin/env bash
# ============================================================
#  SecureVault Backup — Instalador para Ubuntu/Debian Server
# ============================================================
set -e
...
# ── Permisos del script ───────────────────────────────────────
chmod +x "$SCRIPT_DIR/securevault_backup.py"

# ── Crear wrapper ejecutable ──────────────────────────────────
WRAPPER="/usr/local/bin/securevault-backup"
sudo tee "$WRAPPER" > /dev/null <<EOF
#!/usr/bin/env bash
source "$SCRIPT_DIR/venv/bin/activate"
python3 "$SCRIPT_DIR/securevault_backup.py" "\$@"
EOF
sudo chmod +x "$WRAPPER"
echo "[✓] Comando 'securevault-backup' disponible en PATH"
...
echo "  Y añade:  0 2 * * * securevault-backup --config $SCRIPT_DIR/config.yaml >> $SCRIPT_DIR/logs/cron.log 2>&1"
echo ""
