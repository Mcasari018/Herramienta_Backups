...
**Antigravity Backup** es un script Python profesional que automatiza el proceso completo de respaldo:
...
herramienta_backups/
│
├── antigravity_backup.py   # Script principal (toda la lógica)
...
└── logs/                   # Archivos de log (auto-creado)
    └── antigravity_backup.log
```

El archivo `.tar.gz` generado sigue este esquema de nombres:

```
antigravity_backup_<hostname>_<YYYYMMDD_HHMMSS>.tar.gz
```

Ejemplo: `antigravity_backup_webserver01_20260601_020000.tar.gz`

### Estructura interna del .tar.gz

```
antigravity_backup_*.tar.gz
...
cd /opt
sudo mkdir antigravity_backup && sudo chown $USER: antigravity_backup
...
- Registra el comando `antigravity-backup` en `/usr/local/bin`
...
Ejemplos de rutas:
- Linux: `/mnt/nas/backups` o `/var/backups/antigravity`
- Windows: `D:\\Backups\\Antigravity`
...
ssh:
  ...
  remote_dir: "/backups/antigravity"
```
...
ssh-keygen -t ed25519 -C "antigravity-backup" -f ~/.ssh/backup_key
...
```yaml
logging:
  log_file: "./logs/antigravity_backup.log"
...
python antigravity_backup.py
```

### Opciones de línea de comandos

```
antigravity_backup.py [-h] [--config RUTA] [--dry-run] [--version]
...
python3 antigravity_backup.py --config /etc/antigravity/production.yaml

# Simular ejecución completa (dry-run) — ideal para probar la config
python3 antigravity_backup.py --dry-run
...
0 2 * * * /opt/antigravity_backup/venv/bin/python3 /opt/antigravity_backup/antigravity_backup.py --config /opt/antigravity_backup/config.yaml

# Backup cada 6 horas
0 */6 * * * antigravity-backup --config /opt/antigravity_backup/config.yaml

# Backup semanal (domingos a las 03:00)
0 3 * * 0 antigravity-backup
```

**Verificar que cron está ejecutando el backup:**
```bash
# Ver el log del cron del sistema
grep "antigravity" /var/log/syslog

# Ver el log del script
tail -f /opt/antigravity_backup/logs/antigravity_backup.log
```
...
```powershell
# Ver la tarea creada
Get-ScheduledTask -TaskName "AntigravityBackup"

# Ejecutar manualmente la tarea
Start-ScheduledTask -TaskName "AntigravityBackup"
...
Set-ScheduledTask -TaskName "AntigravityBackup" -Trigger $Trigger

# Eliminar la tarea
Unregister-ScheduledTask -TaskName "AntigravityBackup" -Confirm:$false
```

También puedes gestionar la tarea desde la GUI:  
**Inicio → Programador de Tareas → Biblioteca → AntigravityBackup**
...
│       → backups/antigravity_backup_<host>_<timestamp>.tar.gz
│
...
sudo python3 antigravity_backup.py
...
ssh backup_user@servidor_remoto "mkdir -p /backups/antigravity"
```
...
tar -tzf backups/antigravity_backup_*.tar.gz
...
6. **Verifica los backups** regularmente haciendo un restore de prueba:
   ```bash
   # Extraer y verificar
   tar -xzf antigravity_backup_*.tar.gz -C /tmp/restore_test/
   ```
