...
Uso:
    python antigravity_backup.py [--config ruta/config.yaml] [--dry-run]
...
def setup_logging(log_file: str, log_level: str, max_mb: int, backup_count: int) -> logging.Logger:
    ...
    logger = logging.getLogger("antigravity_backup")
...
def apply_retention(backup_dir: str, retention_days: int, logger: logging.Logger, dry_run: bool = False):
    ...
    for f in backup_path.glob("antigravity_backup_*.tar.gz"):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Antigravity Backup — Script de copias de seguridad automatizadas",
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="Antigravity Backup v1.0.0",
    )
...
def main():
    ...
    logger = setup_logging(
        log_file=log_cfg.get("log_file", "./logs/antigravity_backup.log"),
    ...
    try:
        ...
        archive_name = f"antigravity_backup_{hostname}_{timestamp}.tar.gz"
    ...
    if success:
        ...
        subject = f"[Antigravity Backup] ✓ Backup exitoso — {platform.node()}"
        body = (
        ...
        subject = f"[Antigravity Backup] ✗ Backup FALLIDO — {platform.node()}"
        body = (
            f"El backup finalizó con errores.\n\n"
            f"Host: {platform.node()}\n"
            f"Fecha: {datetime.now():%Y-%m-%d %H:%M:%S}\n"
            f"Duración: {elapsed:.1f}s\n\n"
            f"Revisa el log en: {log_cfg.get('log_file', './logs/antigravity_backup.log')}\n"
        )
